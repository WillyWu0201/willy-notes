...
function render() {
  ...
  const rel = related.map((r) =>
    `<a href="/wwdc26/s/${esc(r.id)}${langParam()}"><span class="rid">${esc(r.id)}</span><span class="rt">${esc(r.title)}</span><div class="rg">${esc(lang === "en" ? (r.gloss_en || r.gloss) : r.gloss)}</div></a>`).join("");
